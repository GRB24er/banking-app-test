...
interface Country {
  ...
  // Country-specific bank fields beyond IBAN/SWIFT/sort/routing/account.
  // Example: Canada needs transitNumber + institutionNumber, India needs IFSC, etc.
  extraFields?: BankFieldDef[];
  // Hint shown to the user about what details to gather for this country
  bankInfoHint?: string;
}
...
interface InternationalTransferData {
  ...
  // Country-specific bank fields (transitNumber, ifscCode, clabe, bsbCode, etc.)
...
const FIELD_TRANSIT_CA: BankFieldDef = {
  id: "transitNumber",
...
const FIELD_INSTITUTION_CA: BankFieldDef = {
...
const FIELD_BSB_AU: BankFieldDef = {
  id: "bsbCode",
  label: "BSB Code",
  placeholder: "123-456",
...
const FIELD_IFSC_IN: BankFieldDef = {
  id: "ifscCode",
  label: "IFSC Code",
...
const FIELD_CLABE_MX: BankFieldDef = {
  id: "clabe",
  label: "CLABE",
...
const FIELD_CBU_AR: BankFieldDef = {
  id: "cbu",
  label: "CBU / CVU",
...
const FIELD_CUIT_AR: BankFieldDef = {
  id: "cuit",
  label: "CUIT / CUIL",
...
const FIELD_AGENCY_BR: BankFieldDef = {
  id: "agencyNumber",
  label: "Agência",
  placeholder: "1234",
...
const FIELD_CPF_CNPJ_BR: BankFieldDef = {
  id: "cpfCnpj",
  label: "CPF / CNPJ",
...
const FIELD_PIX_BR: BankFieldDef = {
  id: "pixKey",
...
const FIELD_RUT_CL: BankFieldDef = {
  id: "rut",
  label: "RUT",
...
const FIELD_CCI_PE: BankFieldDef = {
  id: "cci",
  label: "CCI",
...
const FIELD_CNAPS_CN: BankFieldDef = {
  id: "cnaps",
  label: "CNAPS Code",
...
const FIELD_BANK_NAME_CN: BankFieldDef = {
...
const FIELD_BANK_CODE_JP: BankFieldDef = {
  id: "bankCode",
  label: "Bank Code",
  placeholder: "0001",
...
const FIELD_BRANCH_CODE_JP: BankFieldDef = {
  id: "branchCode",
  label: "Branch Code",
  placeholder: "001",
...
const FIELD_ACCOUNT_TYPE_JP: BankFieldDef = {
  id: "accountTypeJP",
  label: "Account Type",
...
const FIELD_BANK_CODE_KR: BankFieldDef = {
  id: "bankCode",
  label: "Bank Code",
  placeholder: "088",
...
const FIELD_BANK_CODE_HK: BankFieldDef = {
  id: "bankCode",
  label: "Bank Code",
  placeholder: "004",
...
const FIELD_BRANCH_CODE_HK: BankFieldDef = {
  id: "branchCode",
  label: "Branch Code",
  placeholder: "123",
  helpText: "3-digit branch code",
...
const FIELD_NUBAN_NG: BankFieldDef = {
  id: "nuban",
...
const FIELD_BRANCH_CODE_ZA: BankFieldDef = {
  id: "branchCode",
  label: "Branch Code",
  placeholder: "632005",
...
const FIELD_BRANCH_CODE_KE: BankFieldDef = {
  id: "branchCode",
  label: "Branch Code",
  placeholder: "12345",
...
const FIELD_BANK_CODE_KE: BankFieldDef = {
  id: "bankCode",
  label: "Bank Code",
  placeholder: "01",
...
const FIELD_BIK_RU: BankFieldDef = {
  id: "bik",
  label: "BIK",
...
const FIELD_CORR_ACCOUNT_RU: BankFieldDef = {
...
const FIELD_INN_RU: BankFieldDef = {
  id: "inn",
  label: "INN (Tax ID)",
...
const FIELD_RIB_MA: BankFieldDef = {
  id: "rib",
  label: "RIB",
...
const FIELD_BD_ROUTING: BankFieldDef = {
  id: "bdRoutingNumber",
...
// Helper: returns a BankFieldDef array describing every required identifier for a country.
// Combines built-in flags (IBAN, sort, routing) with country-specific extraFields.
function getBankFieldsForCountry(code: string): BankFieldDef[] {
  const c = COUNTRIES.find(x => x.code === code);
  if (!c) return [];
  return c.extraFields ? c.extraFields : [];
}
...
export default function InternationalTransferPage() {
  ...
  const handleInputChange = useCallback((field: keyof InternationalTransferData, value: any) => {
    ...
    // Auto-update bank country and clear country-specific bank fields when recipient country changes
    if (field === 'recipientCountry') {
      setFormData(prev => ({
        ...prev,
        bankCountry: prev.bankCountry || value,
        extraBankFields: {}, // reset — different country needs different identifiers
      }));
    }
  ...
  // Handle country-specific extra bank field changes (transitNumber, ifsc, clabe, bsb, etc.)
  const handleExtraBankFieldChange = useCallback((fieldId: string, value: string) => {
    setFormData(prev => ({
      ...prev,
      extraBankFields: { ...prev.extraBankFields, [fieldId]: value },
    }));
    setValidationErrors(prev => prev.filter(e => e.field !== `extra:${fieldId}`));
  }, []);
    ...
    switch (step) {
      ...
      case 3:
        ...
        if (!selectedCountry?.requiresIBAN && !formData.accountNumber) {
          // Some countries replace account number entirely (e.g. Nigeria NUBAN, Mexico CLABE).
          // Only require it if no equivalent extra field is provided.
          const hasReplacement = (selectedCountry?.extraFields || []).some(
            f => ["nuban", "clabe", "cbu", "cci", "rib", "nzAccountFormatted"].includes(f.id) &&
        ...
        (selectedCountry?.extraFields || []).forEach(fdef => {
          const val = (formData.extraBankFields[fdef.id] || "").trim();
          if (fdef.required && !val) {
            errors.push({ field: `extra:${fdef.id}`, message: `${fdef.label} is required` });
            return;
          }
          if (val && fdef.pattern) {
            try {
              ...
              if (!re.test(val.replace(/\s/g, ''))) {
                errors.push({ field: `extra:${fdef.id}`, message: fdef.patternError || `Invalid ${fdef.label}` });
              }
            } catch {
              // Bad regex in config — skip silently rather than blocking the user
            }
          }
        });
  ...
  const applyTemplate = (templateId: string) => {
    ...
    if (template && template.data) {
      ...
      const newFormData: InternationalTransferData = {
        ...
        extraBankFields: template.data.extraBankFields || formData.extraBankFields,
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const internationalTransferData = {
        ...
        // Country-specific bank identifiers (transit/institution, IFSC, CLABE, BSB, etc.)
        extraBankFields: formData.extraBankFields,
      };
            ...
            <AnimatePresence mode="wait">
              ...
              {currentStep === 3 && (
                ...
                >
                  ...
                  {(() => {
                    const sel = COUNTRIES.find(c => c.code === formData.recipientCountry);
                    return (
                        ...
                        <div>
                          ...
                          {sel?.bankInfoHint && (
                            <p style={{ margin: '8px 0 0', fontWeight: 600 }}>
                              {sel.emoji} {sel.name}: {sel.bankInfoHint}
                            </p>
                          )}
                          {!sel?.bankInfoHint && sel && (
                            <p style={{ margin: '8px 0 0' }}>
                              {sel.requiresIBAN
                                ? 'IBAN is required for this country.'
                                : 'Account number is required for this country.'}
                            </p>
                          )}
                        </div>
                      </div>
                  ...
                  <div className={styles.formGrid}>
                    ...
                    {/* Country-specific bank identifiers (transit/institution for Canada,
                      ...
                      return (
                        <div key={fdef.id} className={styles.formField}>
                          ...
                          <input
                            ...
                            className={getFieldError(errKey) ? styles.error : ''}
                          />
                          {fdef.helpText && (
                          ...
                          {getFieldError(errKey) && (
                            <span className={styles.errorMessage}>{getFieldError(errKey)}</span>
                          )}
                        </div>
                      );
                    })}
                    ...
                    <div className={styles.reviewCard}>
                      ...
                      <div className={styles.reviewDetails}>
                        ...
                        {getBankFieldsForCountry(formData.recipientCountry).map(fdef => {
                          ...
                          if (!v) return null;
                          return (
                            <div key={fdef.id} className={styles.detailRow}>
